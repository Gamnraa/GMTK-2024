...
func _process(delta):
	...
	#	if $Area2D.overlaps_body(ball) and not $AnimationPlayer.is_playing():
	#		$Timer.wait_time = randf_range(0.0, 0.34)
	#		$Timer.start()
	#		$AnimationPlayer.call_deferred(calls[next_anim], "Activate" + sides[side])
...
func _on_area_2d_body_entered(body):
	if body is RigidBody2D and not $AnimationPlayer.is_playing():
		...
		$Timer.wait_time = randf_range(0.15, 0.34)
		$Timer.start()
...
func _on_animation_player_animation_finished(anim_name):
	if next_anim == 0:
		next_anim = 1
		$Timer.wait_time = randf_range(0.0, 1.1)
		$Timer.start()
	else: next_anim = 0
